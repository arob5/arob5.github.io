...
$B^\top g$. The question is now: how do we find the basis vectors $B$?
...
underlying goal here is to approximate the map $u \mapsto \mathcal{G}(u)$.
We thus need to
consider how to leverage the basis representation of the output space in achieving
this goal. Assuming we have already constructed the basis vectors $B$, the
map $u \mapsto \mathcal{G}(u)$ can be approximated as
$$
...
$B^\top \mathcal{G}(u)$ stores the $r$ weights defining the projection of
$\mathcal{G}(u)$ onto the
