...
\hat{C}\_{k+1}^{y} &:= \frac{1}{J-1} \sum_{j-1}^{J} (y\_{k+1}^{(j)}-\overline{y}\_{k+1})(y\_{k+1}^{(j)}-\overline{y}\_{k+1})^\top,
\end{align}
...
$$
v^{(j)}_{k+1} :=
v^{(j)}_{k} + \hat{C}_{k+1}^{vy}[\hat{C}_{k+1}^{y}]^{-1}(y_{k+1} - y_{k+1}^{(j)}). \tag{12}
$$
...
<blockquote>
  ...
  \begin{align}
  &v^{(j)}_{k+1} :=
  v^{(j)}_{k} + \hat{C}_{k+1}^{vy}[\hat{C}_{k+1}^{y}]^{-1}(y_{k+1} - y_{k+1}^{(j)}),
  &&y_{k+1}^{(j)} \sim \mathcal{N}(h(\hat{v}_{k+1}^{(j)}),R) \tag{14}
  \end{align}
...
\begin{bmatrix} \hat{C}\_{k+1} & \hat{C}\_{k+1}H^\top \newline
                H\hat{C} & H\hat{C}\_{k+1}H^\top + R \end{bmatrix}
\right). \tag{10}
\end{align}
...
by (10) to obtain the analysis update. This update, the analog of (14), is
...
<blockquote>
  ...
  \begin{align}
  &v^{(j)}_{k+1} :=
  v^{(j)}_{k} + \hat{C}_{k+1}H^\top[H\hat{C}_{k+1}H^\top + R]^{-1}(y_{k+1} - y_{k+1}^{(j)}),
  &&y_{k+1}^{(j)} \sim \mathcal{N}(H\hat{v}_{k+1}^{(j)},R) \tag{14}
  \end{align}
