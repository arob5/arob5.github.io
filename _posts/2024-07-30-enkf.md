...
surprising that this update boils down to (i) using the dynamical model
to forecast one time step ahead, and then (ii) conditioning on the new data
point. We can thus decompose the map $\pi_k \mapsto \pi_{k+1}$ as
$$
\pi_k \mapsto \hat{\pi}_{k+1} \mapsto \pi_{k+1}, \tag{5}
$$
with the two arrows encoding steps (i) and (ii), respectively. In the linear Gaussian
...
Bayesian filtering distributions, and the following subsection reinterprets the
algorithm from an optimization perspective.
{% endkatexmm %}
...
approximation of (8) with mean and covariance set to the empirical mean and
covariance of the particles
$\{(\hat{v}_{k+1}^{(j)}, \hat{y}_{k+1}^{(j)})\}_{j=1}^{J}$, where
...
Note that the notation $\hat{y}_{k+1}^{(j)}$ indicates that this quantity
...
&\overset{d}{\approx} \mathcal{N}(\tilde{m}_{k+1}, \tilde{C}_{k+1}) :=
\mathcal{N}\left(
...
$(v^{(j)}_{k+1})_{j=1}^{J}$. The third method is the one actually employed by
the EnKF.

#### Gaussian Conditional Moments
Perhaps the most straightforward idea is to apply the typical Gaussian
conditioning identities to the joint Gaussian
$\mathcal{N}(\tilde{m}_{k+1}, \tilde{C}_{k+1})$ defined in $(10).
The conditional mean and covariance are given by

\begin{align}
m_{k+1} &= \hat{m}\_{k+1} + \hat{C}^{vy}\_{k+1}[\hat{C}^y\_{k+1}]^{-1}(y_{k+1}-\hat{y}_{k+1}) \tag{12} \newline
...
moments. Matheron's rule provides a way to map samples from the joint
distribution $\mathcal{N}(\tilde{m}_{k+1}, \tilde{C}_{k+1})$ to conditional
samples:
$$
v^{(j)}_{k+1} :=
\tilde{v}^{(j)} + \hat{C}_{k+1}^{vy}[\hat{C}_{k+1}^{y}]^{-1}(y_{k+1} - \tilde{y}^{(j)}),
\qquad (\tilde{v}^{(j)},\tilde{y}^{(j)}) \sim \mathcal{N}(\tilde{m}_{k+1}, \tilde{C}_{k+1}). \tag{13}
$$
The resulting ensemble $(v_{k+1}^{(j)})_{j=1}^{J}$ consists of particles
distributed according to the Gaussian with moments (12); in other words, this is
simply a different algorithm for obtaining the same ensemble as in the previous
method.
...
<blockquote>
  ...
  \begin{align}
  &v^{(j)}_{k+1} :=
  \hat{v}^{(j)}_{k+1} + \hat{C}_{k+1}H^\top[H\hat{C}_{k+1}H^\top + R]^{-1}(y_{k+1} - \hat{y}_{k+1}^{(j)}),
