...
<blockquote>
  ...
  \begin{align}
  &v^{(j)}_{k+1} :=
  v^{(j)}_{k} + \hat{C}_{k+1}^{vy}[\hat{C}_{k+1}^{y}]^{-1}(y_{k+1} - \hat{y}_{k+1}^{(j)}),
...
<blockquote>
  ...
  \begin{align}
  &v^{(j)}_{k+1} :=
  v^{(j)}_{k} + \hat{C}_{k+1}H^\top[H\hat{C}_{k+1}H^\top + R]^{-1}(y_{k+1} - \hat{y}_{k+1}^{(j)}),
...
$$
So we can alternatively define $y^{(j)}_{k+1} := y_{k+1} + \epsilon_{k+1}^{(j)}$,
and indeed both conventions exist in the literature. 
