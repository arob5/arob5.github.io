...
\begin{align}
y_i|\beta &\sim \mathcal{N}(x_i^\top \beta, \sigma^2),
\end{align}
meaning that the magitude of the observation noise is iid across observations.
...
\end{align}
where we have also assumed a Gaussian prior on $\beta$. Thus, the forward model
$G = X$ is given by the data matrix, while the parameter $u = \beta$ is the
coefficient vector. The specification of the prior covariance as $\sigma^2 C$ is
