...
subtitle: A brief introduction to Gaussian measures in one dimension.
date: 2024-05-16
categories: [GP, Prob-Theory]
format:
  html:
    css: ../../styles.css
...
about densities to thinking more abstractly in terms of measures. While this 
may seem unnecessarily pedantic in one dimension, we will find that the
...
$$
\mathcal{N}(x \mid m, \sigma^2) := \frac{1}{\sqrt{2\pi \sigma^2}} \exp\left\{-\frac{1}{2\sigma^2}(x - m)^2 \right\}. \tag{1}
$$
We're typically used to defining the Gaussian as a random variable with density
equal to $\mathcal{N}(x \mid m, \sigma^2)$. Since
...
$\mathcal{N}(x \mid m, \sigma^2)$; i.e.,
\begin{align}
\frac{d\mu}{d\lambda}(x) = \mathcal{N}(x \mid m, \sigma^2).
\end{align}
The density, of course, is only defined if $\sigma^2 > 0$. It turns out to be
nice to also allow for the $\sigma^2 = 0$ case. While $\mathcal{N}(x \mid m, 0)$
...
while Gaussian measures that admit densities are *non-degenerate*.
We write $\mu = \mathcal{N}(m, \sigma^2)$ to signify that $\mu$ is a Gaussian
measure with density $\mathcal{N}(x \mid m, \sigma^2)$ if $\sigma^2 > 0$, or
$\mu = \delta_m$ if $\sigma^2 = 0$. When $m = 0$ we call $\mu$
*centered* or *symmetric*.
Note that the measure $\mu$ is symmetric in the sense that $\mu(B) = \mu(-B)$
for any Borel set $B$.
...
\begin{align}
&\int x \delta_m(dx) = m, &&\int [x - m]^2 \delta_m(dx) = [m - m]^2 = 0.
\end{align}
...
the standard Fourier transform, which acts on functions, to an operator that
acts on measures. Probability theorists draw a distinction between
...
The quantity $\mathbb{P} \circ X^{-1}$ is
the *pushforward* of the measure $\mathbb{P}$ under the map $X$, given by
\begin{align}
