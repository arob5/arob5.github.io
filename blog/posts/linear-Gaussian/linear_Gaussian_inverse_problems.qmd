...
description: Derivations and discussion of linear Gaussian inverse problems.
layout: default
date: 2024-07-03
categories: [statistics, inverse-problem]
format:
  html:
    css: ../../styles.css
...
1. Linear forward model.
2. Additive Gaussian observation noise.
3. Gaussian prior distribution.
4. Prior independence of the observation noise and prior.
...
::: {.callout-note title="Linear Gaussian Model}
A linear Gaussian model is a joint probability distribution on data
$y \in \R^n$ and a parameter $u \in \R^d$ taking the form
$$
\begin{align}
y \mid u &\sim \Gaussian(Gu, \Sigma) \\
u &\sim \Gaussian(m, C),
\end{align}
$$ {#eq-inv-prob}

Here, $G \in \mathbb{R}^{n \times d}$ is a linear forward model mapping the
parameter to the observable. The observation covariance
$\Sigma \in \R^{n \times n}$ and prior covariance $C \in \R^{d \times d}$
are both fixed positive definite matrices.

The model can equivalently be written by introducing another random variable
$\noise$ to explicitly represent the observation noise
$$
\begin{align}
y &= Gu + \epsilon \newline
\noise &\sim \mathcal{N}(0, \Sigma) \\
u &\sim \mathcal{N}(m, C), && u \perp \epsilon,
\end{align}
$$ {#eq-inv-prob-2}

where $u \perp \epsilon$ is used to indicate the key assumption that
$u$ and $\epsilon$ are a priori probabilistically independent.
:::

Upon observing a particular realization $\yobs$ of the data, the Bayesian
inverse problem is solved by computing the **posterior distribution**
$$
\begin{align}
p(u \mid y = \yobs)
&= \frac{1}{Z} \Gaussian(u \mid m, C) \Gaussian(y \mid Gu, \Sigma),
&& Z = \int \Gaussian(u \mid m, C) \Gaussian(\yobs \mid Gu, \Sigma) du.
\end{align}
$$

The ubiquity of the linear Gaussian models stems from its analytic tractability;
the assumptions of linearity and Gaussianity play nicely together, yielding
a closed-form characterization of the posterior. In the following section,
we derive the posterior in two different ways, each providing a different
(but equivalent) solution.

# Deriving the Posterior Distribution
In this section, we will find that the linear Gaussian assumptions combine
to produce a Gaussian posterior.
In Bayesian statistics, this is called a *conjugate* model, as the prior and
posterior belong to the same distributional family. Once it is
...
in $\R^d$ rather than $\R^n$. In the first set of equations, the
