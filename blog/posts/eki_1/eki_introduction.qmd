...
$\u \given \yobs$ under the model in @eq-bip. We can phrase this as the task
...
@eq-misfit-post is precisely the least squares misfit function from @eq-misfit
(up to an additive constant). Moreover, if we choose a Gaussian prior
...
certain algorithms that assume the prior is Gaussian, but in general
allow $\priorDens$ to be non-Gaussian.
...
derivative-free, suitable for the black-box setting where we can only
...
are the cross-covariances between $\u$ and $\y$.
...
As the conditional of a Gaussian
...
The assumption from the preceding section yielded the analytically
tractable approximation in @eq-Gaussian-cond. In the algorithms we are building
towards, we will require a Monte Carlo representation of the posterior
approximation. To build towards this approach,
let's first review how we might obtain a Monte Carlo representation of
the Gaussian posterior in @eq-Gaussian-cond.
With the closed-form approximation from @eq-Gaussian-cond in hand, we can
of course simply compute $\meanPost$ and $\covPost$ using the above
equations, then sample $\Gaussian(\meanPost, \covPost)$ using standard
methods. Interestingly, we can actually bypass the step of computing
conditional moments and directly sample from the Gaussian conditional using a
result known as **Matheron's rule**.
...
Note that this map depends only on the first two moments of $\joint$.
...
@eq-Matheron-transport. Sampling $(u^\prime, \y^\prime)$
...
the observed and predicted data). Note that the Kalman gain is a linear
map that operates in the observation space (i.e., $\R^n$).
...
$\postDens$. We consider the following basic tempering schedule,
starting with a generic result for two arbitrary densities, and then
specializing to our particular setting.
...
The following corollary specializes the result
