...
$$
\misfit(u) := \frac{1}{2}\lVert \yobs - \fwd(\u)\rVert^2_{\covObs} := \frac{1}{2}(\yobs - \fwd(\u))^\top \covObs^{-1}(\yobs - \fwd(\u)),
$$ {#eq-misfit}
weighted by a positive definite matrix $\covObs$. The (nonlinear) least squares
...
$$
\misfitReg(\u) := \frac{1}{2}\lVert \yobs - \fwd(\u)\rVert^2_{\covObs} + \frac{1}{2}\lVert \u - m\rVert^2_{\covMat}.
$$ {#eq-misfit-reg}
...
requirement to keep $\u$ "close" to $m$, where the relative weights of these
objectives are determined by the (positive definite) covariances
$\covObs$ and $\covMat$.
...
\y &= \fwd(\u) + \noise \newline
\u &\sim \priorDens \newline
\noise &\sim \Gaussian(0, \covObs),
\end{align}
$$ {#eq-bip}
where $\priorDens$ is a prior distribution on the parameter, and $\covObs$
is the fixed (known) covariance of the additive Gaussian noise. The EnKF
...
on $(\u,\y)$ via the product of densities
$$
p(\u,\y) := p(\y \given \u) \priorDens(\u) = \Gaussian(\y \given \fwd(\u), \covObs)\priorDens(\u),
$$ {#eq-joint-dens}
with the posterior density given by Bayes' theorem
$$
\begin{align}
\postDens(\u) &:= p(\u \given \y) = \frac{1}{\normCst}\Gaussian(\yobs \given \fwd(\u), \covObs)\priorDens(\u),
&&\normCst := \int_{\parSpace} \Gaussian(\yobs \given \fwd(\u), \covObs)\priorDens(\u) d\u.
\end{align}
$$ {#eq-Bayes-thm}
We omit the dependence on $\yobs$ in the notation $\postDens(\u)$ and $Z$.

::: {.callout-note title="Goal 3: Posterior Sampling"}
The sampling goal is to draw samples from the posterior distribution
...
= \frac{1}{2}\lVert \yobs - \fwd(\u)\rVert^2_{\covObs} - \log \priorDens(\u) + C,
$$ {#eq-misfit-post}
...
$\priorDens(\u) = \Gaussian(\u \given m, \covMat)$, then $\misfitPost(\u)$
agrees with $\misfitReg(\u)$, up to an additive constant. We will explore
