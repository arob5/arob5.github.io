...
interpolate between two probability distributions, but we will start by
considering the following basic tempering schedule.
...
::: {.callout-note title="Prior-to-Posterior Map in Finite Time"}
Let $\priorDens(\u)$ and $\postDens(\u) = \frac{1}{\normCst}$ be two probability
densities on $\parSpace$. For a positive integer $K$, define the sequence of
densities $\dens_0, \dens_1, \dots, \dens_K$ recursively by
$$
\begin{align}
\dens_{k+1}(\Par) &:= \frac{1}{\normCst_{k+1}}\dens_k(\Par)\exp\left(-\frac{1}{K}\Phi(u)\right),
&&\normCst_{k+1} := \int \dens_k(\Par)\exp\left(-\frac{1}{K}\Phi(\Par)\right) d\Par,
\end{align}
$$
for $k = 0, \dots, K-1$. Then the final density satisfies $\dens_K = \postDens$,
where $\postDens$ is defined in @bip.
:::
